    ...
    try {
      ...
      if (editingResident) {
        ...
        if (newStatus === 'Moved On' && originalStatus === 'Active' && originalAccommodationId) {
          ...
          const allNames = [...others.map(o => `${o["First Name"] || o.first_name} ${o["Last Name"] || o.last_name}`.trim()), `${savedResident["First Name"]} ${savedResident["Last Name"]}`.trim()];
          const hasAllocated = (allocatedResidents || []).some(ar =>
        ...
        if (savedResident.Status === 'Active' && savedResident["Accommodation ID"]) {
          ...
          const allNames = [
            ...othersInRoom.map(r => `${r["First Name"] || r.first_name} ${r["Last Name"] || r.last_name}`.trim()),
            `${savedResident["First Name"]} ${savedResident["Last Name"]}`.trim()
          ];
  ...
  const exportToCSV = () => {
    ...
    try {
      ...
      const rows = filteredResidents.map(resident => [
        ...
        resident["Sign Up Google Drive Link"] || resident["Google Drive Link"] || "",
        resident["Resident Photographic ID Link (Google Drive)"] || resident["Resident Photographic Link"] || "",
        resident["Photo Of Individual (Google Drive)"] || resident["Photo ID URL"] || "",
        resident.Notes || "",
        resident["Has No PA"] ? "Yes" : "No",
