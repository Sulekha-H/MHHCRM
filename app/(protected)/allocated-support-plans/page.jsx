...
const normalizeData = (data) => {
  ...
  // Alias fields for compatibility with shared components
...
export default function AllocatedSupportPlans() {
  const { user } = useUser();
...
const filterPlans = useCallback(() => {
  ...
  current = current.filter(plan => {
    ...
    // ONLY show plans for residents that are in our allocated residents list
...
useEffect(() => {
  ...
  const exportSupportNotesToCSV = () => {
    const supportNotesData = supportPlans.filter(plan => {
      if (plan.plan_type !== 'support_notes' || plan.deleted || !plan.resident_id || plan.resident_id.trim() === '') return false;
      // Only include if resident is in our allocated residents list
      return residents.some(r => r.id === plan.resident_id);
    });
  ...
  const exportQuarterlyReviewsToCSV = () => {
    const quarterlyReviewsData = supportPlans.filter(plan => {
      if (plan.plan_type !== 'quarterly_reviews' || plan.deleted || !plan.resident_id || plan.resident_id.trim() === '') return false;
      // Only include if resident is in our allocated residents list
      return residents.some(r => r.id === plan.resident_id);
    });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Allocated Support Plans & Notes</h1>
          <p className="text-slate-600">Track daily support notes and quarterly reviews for allocated residents</p>
        </div>
        ...
        <TabsContent value="quarterly_reviews" className="mt-6 space-y-6">
          <Card>
            <CardHeader>
              ...
              <p className="text-slate-600">Monthly overview of upcoming and completed quarterly reviews (Next 12 Months)</p>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto border rounded-lg">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="sticky left-0 bg-white z-20 min-w-[200px]">Resident Name</TableHead>
                      {(() => {
                        const rollingMonths = getRolling12Months();
                        return rollingMonths.map(monthDate => (
                          <TableHead key={monthDate.toISOString()} className="text-center min-w-[120px]">
                  ...
                  <TableBody>
                    {(() => {
                      ...
                      return residents
                        ...
                        .map(resident => {
                          const rawMoveInDate = resident.move_in_date || resident['move-in_date'] || resident['Move-in Date'] || resident['Move In Date'] || resident['Move-In Date'];
                              ...
                              return (
                                <TableCell
                                  ...
                                  className={`text-center p-2 border-r last:border-r-0 ${
                                    status === 'due' ? 'bg-red-50/50' :
                                    status === 'overdue' ? 'bg-red-100/50' :
                                ...
                                >
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    className="w-full h-auto py-2 flex flex-col gap-1 hover:bg-slate-100 transition-colors"
                                    onClick={() => {
                                      if (reviewInMonth) {
                                  ...
                                  >
                                    {status === 'completed' ? (
                                      ...
                                      <>
                                        <Badge variant="destructive" className="bg-red-600 hover:bg-red-700 animate-pulse text-[10px] py-0 px-1">DUE</Badge>
