...
export async function injectRoutineTasks(supabase, user, existingTasks, targetDate = new Date()) {
  ...
  for (const routine of routineTasksToConsider) {
    ...
    // Check if it already exists in the database tasks (check both Title and title casing)
    ...
    if (!alreadyInDB && !alreadyInList) {
      // Encode timing metadata into the Description field since schema columns are missing
      const metadata = {
