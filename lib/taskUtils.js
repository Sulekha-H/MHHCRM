...
export async function injectRoutineTasks(supabase, user, existingTasks, targetDate = new Date()) {
  ...
  const routineTasksForDay = allRoutineTasksForDay.filter(r =>
    !r.assignedTo || (userName && r.assignedTo.toLowerCase() === userName.toLowerCase())
  );

  if (routineTasksForDay.length === 0) return;
