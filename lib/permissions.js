...
export const SUPPORT_WORKER_EMAILS = [].map(email => email.toLowerCase());
...
/**
 * Checks if a user is a support worker (has extra pages on top of restricted access)
 */
export function isSupportWorker(user) {
  if (!user?.emailAddresses?.[0]?.emailAddress) return false;
  const userEmail = user.emailAddresses[0].emailAddress?.trim().toLowerCase();
  return SUPPORT_WORKER_EMAILS.includes(userEmail);
}

/**
 * Checks if a user is Jess (Jessica)
 */
export function isJess(user) {
  if (!user?.emailAddresses?.[0]?.emailAddress) return false;
  const userEmail = user.emailAddresses[0].emailAddress?.trim().toLowerCase();
  return userEmail === 'jessica@myhopehousing.org.uk';
}
